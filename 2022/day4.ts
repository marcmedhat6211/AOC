...
const getRangeContainsTheOtherPairCount = (pairs: string[][]): number => {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    if (
      (fomattedPair.firstRange.first >= fomattedPair.secondRange.first &&
        fomattedPair.firstRange.second <= fomattedPair.secondRange.second) ||
      (fomattedPair.secondRange.first >= fomattedPair.firstRange.first &&
        fomattedPair.secondRange.second <= fomattedPair.firstRange.second)
    ) {
      count++;
    }
  }
 ...
 *
 * case:
 *  in second range of the pair: if first <= second
 *  A start <= B end and B start <= A end
 *  if firstRange.first <= secondRange.second and secondRange.first <= firstRange.second
 *
 * input: array of arrays, each subarray contains a string similar to '2-4,6-8'
 */

const getOverlappingPairs = (pairs: string[][]): number => {
  let overlappingPairs = 0;

  for (let i = 0; i < pairs.length; i++) {
    const fomattedPair = formatPair(pairs[i]);
    const { firstRange, secondRange } = fomattedPair;

    if (
      firstRange.first <= secondRange.second
      &&
      secondRange.first <= firstRange.second
    ) {
      overlappingPairs++;
    }
  }
